...
#+begin_src emacs-lisp
  ...
  ;; Ensure Org is loaded before tweaking faces/hooks that depend on it
  (require 'org)
  (require 'org-indent)
#+end_src
...
*** Type and size
#+begin_src emacs-lisp
  ;; Set default monospace font for code
  (set-face-attribute 'default nil
    :family "Maple Mono NF"
    :height 180
    :weight 'regular)

  ;; Use EB Garamond as proportional font for text/Org
  (set-face-attribute 'variable-pitch nil
    :family "EB Garamond"
    :height 180
    :weight 'regular)

  ;; Ensure fixed-pitch face stays monospace
  (set-face-attribute 'fixed-pitch nil
    :family "Maple Mono NF"
    :height 180
    :weight 'regular)

  ;; Prefer Maple Mono NF for symbols/emoji so Nerd glyphs render while text stays EB Garamond
  (dolist (charset '(symbol emoji))
    (set-fontset-font t charset (font-spec :family "Maple Mono NF") nil 'prepend))

  ;; Enable variable-pitch in text and Org buffers
...
*** Symbols and emoji
#+begin_src emacs-lisp
  ;; Prefer Nerd Font glyphs for symbols/emoji while keeping Garamond text
  (dolist (charset '(symbol emoji))
    (set-fontset-font t charset (font-spec :family "Maple Mono NF") nil 'prepend))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org
    ...
    ;; Keep code, blocks and tables monospace even in variable-pitch Org buffers
  ...
  ;; Keep code, tables, and meta lines monospace inside Org
